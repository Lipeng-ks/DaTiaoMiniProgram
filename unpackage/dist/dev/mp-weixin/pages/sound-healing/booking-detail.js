...
const _sfc_main = {
  ...
  methods: {
    async loadBookingDetail() {
      this.loading = true;
      try {
        this.booking = await services_booking_service.BookingService.getBookingDetail(this.bookingId);
        this.checkCanCancel();
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/sound-healing/booking-detail.vue:123", "Failed to load booking detail:", error);
    ...
    checkCanCancel() {
      ...
      this.canCancelBooking = services_booking_service.BookingService.canCancel(this.booking);
    },
    async handleCancel() {
    ...
    async cancelBooking() {
      try {
        const result = await services_booking_service.BookingService.cancelBooking(this.bookingId);
        if (result.success) {
          common_vendor.index.showToast({
            title: "取消成功",
            icon: "success"
        ...
        common_vendor.index.__f__("error", "at pages/sound-healing/booking-detail.vue:171", "Failed to cancel booking:", error);
        common_vendor.index.showToast({
          title: "取消失败",
          icon: "none"
        });
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    f: common_vendor.t($data.booking.eventTitle),
    g: common_vendor.t($data.booking.eventDate),
    h: common_vendor.t($data.booking.eventTime),
    i: common_vendor.t($data.booking.eventLocation),
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-19766aaa"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../.sourcemap/mp-weixin/pages/sound-healing/booking-detail.js.map
