...
const _sfc_main = {
  ...
  methods: {
    async loadBookingDetail() {
      this.loading = true;
      try {
        this.booking = await services_artBooking_service.ArtBookingService.getArtBookingDetail(this.bookingId);
        this.checkCanCancel();
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/art-healing/booking-detail.vue:123", "Failed to load booking detail:", error);
    ...
    checkCanCancel() {
      ...
      this.canCancelBooking = services_artBooking_service.ArtBookingService.canCancelArtBooking(this.booking);
    },
    async handleCancel() {
    ...
    async cancelBooking() {
      try {
        const result = await services_artBooking_service.ArtBookingService.cancelArtBooking(this.bookingId);
        if (result.success) {
          common_vendor.index.showToast({
            title: "取消成功",
            icon: "success"
        ...
        common_vendor.index.__f__("error", "at pages/art-healing/booking-detail.vue:171", "Failed to cancel booking:", error);
        common_vendor.index.showToast({
          title: "取消失败",
          icon: "none"
        });
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    f: common_vendor.t($data.booking.festivalTitle),
    g: common_vendor.t($data.booking.festivalDate),
    h: common_vendor.t($data.booking.festivalTime),
    i: common_vendor.t($data.booking.festivalLocation),
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-df8dd129"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../.sourcemap/mp-weixin/pages/art-healing/booking-detail.js.map
