      ...
      <view class="info-section">
        ...
        <view class="info-card">
          <view class="info-row">
            ...
            <text class="info-value">{{ booking.festivalTitle }}</text>
          </view>
          <view class="info-row">
            <text class="info-label">活动日期</text>
            <text class="info-value">{{ booking.festivalDate }}</text>
          </view>
          <view class="info-row">
            <text class="info-label">活动时间</text>
            <text class="info-value">{{ booking.festivalTime }}</text>
          </view>
          <view class="info-row">
            <text class="info-label">活动地点</text>
            <text class="info-value">{{ booking.festivalLocation }}</text>
          </view>
        </view>
      </view>
...
export default {
  ...
  methods: {
    async loadBookingDetail() {
      this.loading = true
      try {
        // Load booking detail from ArtBookingService
        this.booking = await ArtBookingService.getArtBookingDetail(this.bookingId)
        this.checkCanCancel()
      } catch (error) {
    ...
    checkCanCancel() {
      ...
      // Use ArtBookingService.canCancelArtBooking to check 24-hour rule
      this.canCancelBooking = ArtBookingService.canCancelArtBooking(this.booking)
    },
    async handleCancel() {
      uni.showModal({
        title: '确认取消',
    ...
    async cancelBooking() {
      try {
        // Use ArtBookingService to cancel booking with 24-hour rule validation
        const result = await ArtBookingService.cancelArtBooking(this.bookingId)
