      ...
      <view class="info-section">
        ...
        <view class="info-card">
          <view class="info-row">
            ...
            <text class="info-value">{{ booking.salonTitle }}</text>
          </view>
          <view class="info-row">
            <text class="info-label">活动日期</text>
            <text class="info-value">{{ booking.salonDate }}</text>
          </view>
          <view class="info-row">
            <text class="info-label">活动时间</text>
            <text class="info-value">{{ booking.salonTime }}</text>
          </view>
          <view class="info-row">
            <text class="info-label">活动地点</text>
            <text class="info-value">{{ booking.salonLocation }}</text>
          </view>
        </view>
      </view>
...
export default {
  ...
  methods: {
    async loadBookingDetail() {
      this.loading = true
      try {
        // Load booking detail from AromaBookingService
        this.booking = await AromaBookingService.getAromaBookingDetail(this.bookingId)
        this.checkCanCancel()
      } catch (error) {
    ...
    checkCanCancel() {
      ...
      // Use AromaBookingService.canCancelAromaBooking to check 24-hour rule
      this.canCancelBooking = AromaBookingService.canCancelAromaBooking(this.booking)
    },
    async handleCancel() {
      uni.showModal({
        title: '确认取消',
    ...
    async cancelBooking() {
      try {
        // Use AromaBookingService to cancel booking with 24-hour rule validation
        const result = await AromaBookingService.cancelAromaBooking(this.bookingId)
...
.header {
  background: linear-gradient(135deg, #ad1457 0%, #e91e63 60%, #f48fb1 100%);
  padding: 48rpx 40rpx;
  text-align: center;
  position: relative;
  overflow: hidden;
