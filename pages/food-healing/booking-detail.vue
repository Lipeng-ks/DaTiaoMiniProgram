      ...
      <view class="info-section">
        ...
        <view class="info-card">
          <view class="info-row">
            ...
            <text class="info-value">{{ booking.experienceTitle }}</text>
          </view>
          <view class="info-row">
            <text class="info-label">活动日期</text>
            <text class="info-value">{{ booking.experienceDate }}</text>
          </view>
          <view class="info-row">
            <text class="info-label">活动时间</text>
            <text class="info-value">{{ booking.experienceTime }}</text>
          </view>
          <view class="info-row">
            <text class="info-label">活动地点</text>
            <text class="info-value">{{ booking.experienceLocation }}</text>
          </view>
        </view>
      </view>
...
export default {
  ...
  methods: {
    async loadBookingDetail() {
      this.loading = true
      try {
        // Load booking detail from FoodBookingService
        this.booking = await FoodBookingService.getFoodBookingDetail(this.bookingId)
        this.checkCanCancel()
      } catch (error) {
    ...
    checkCanCancel() {
      ...
      // Use FoodBookingService.canCancelFoodBooking to check 24-hour rule
      this.canCancelBooking = FoodBookingService.canCancelFoodBooking(this.booking)
    },
    async handleCancel() {
      uni.showModal({
        title: '确认取消',
    ...
    async cancelBooking() {
      try {
        // Use FoodBookingService to cancel booking with 24-hour rule validation
        const result = await FoodBookingService.cancelFoodBooking(this.bookingId)
...
.header {
  background: linear-gradient(135deg, #e65100 0%, #ff8f00 60%, #ffb74d 100%);
  padding: 48rpx 40rpx;
  text-align: center;
  position: relative;
  overflow: hidden;
